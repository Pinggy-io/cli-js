...
export function openDownloadPage() {
    ...
    exec(command, (err) => {
        if (err) {
            ...
            CLIPrinter.info("Please visit the following page to download the runtime:\n");
            CLIPrinter.info(url + "\n");
            return;
        }

        CLIPrinter.info("\nOpening Microsoft download page in your browser...");
        CLIPrinter.info("Please install the Visual C++ Runtime and restart this application.\n");
    })
}
...
export function getVCRedistMessage() {
    ...
    return {
        ...
        message:
            "This application requires the Microsoft Visual C++ Runtime to run on Windows.\n\n" +
